...
for(j in 1:length(sheets)){
  ...
  df_Annual <- df_Annual[df_Annual$EDGAR >= 100, ] # drop values less than 100
  # Drop NA values specifically for column 'A'
  df_Annual <- df_Annual[!is.na(df_Annual$Obs), ]
  df_Annual <- df_Annual[!is.na(df_Annual$ODIAC), ]
...
write.xlsx2(finalDf, 
           file = "E:/Eeshan/evaluation/results/results_Annual_d01_d02.xlsx", 
           sheetName= "2017", 
           append = TRUE)
