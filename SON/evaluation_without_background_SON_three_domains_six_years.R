...
for(j in 1:length(sheets)){
  ...
  df_SON_initial <- rbind(df_SON_initial_1,df_SON_initial_2,df_SON_initial_3,df_SON_initial_4,df_SON_initial_5,df_SON_initial_6)
  
  time <- df_SON_initial[, 1]
  df_without_time <- df_SON_initial[, -1]
  test<- df_without_time[] - 365.0 #substracting pre-industrial values 
  options(digits = 5) #changing the decimal places 
  df_SON <- data.frame(time, test)
  ...
  obs <- dplyr::pull(df_SON, 2) #changing it to numeric
  EDGAR_d03 <- df_SON[,11]
  EDGAR_d03 <- dplyr::pull(df_SON,11)
...
write.xlsx2(finalDf, 
            file = "E:/Eeshan/evaluation/results/results_without_background_SON_d01_d02_d03.xlsx", 
            sheetName="Average_6", 
            append=TRUE)
