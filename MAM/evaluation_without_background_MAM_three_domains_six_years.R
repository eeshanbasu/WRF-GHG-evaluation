...
for(j in 1:length(sheets)){
  ...
  df_MAM_initial <- rbind(df_MAM_initial_1,df_MAM_initial_2,df_MAM_initial_3,df_MAM_initial_4,df_MAM_initial_5,df_MAM_initial_6)
  
  time <- df_MAM_initial[, 1]
  df_without_time <- df_MAM_initial[, -1]
  test<- df_without_time[] - 375.0 #substracting pre-industrial values 
  options(digits = 5) #changing the decimal places 
  df_MAM <- data.frame(time, test)
  df_MAM <- df_MAM[df_MAM$EDGAR >= 0, ] # drop negative values
  df_MAM <- df_MAM[df_MAM$ODIAC >= 0, ] # drop negative values
  df_MAM <- df_MAM[df_MAM$EDGAR_d02 >= 0, ] # drop negative values
  df_MAM <- df_MAM[df_MAM$ODIAC_d02 >= 0, ] # drop negative values
  ...
  obs <- dplyr::pull(df_MAM, 2) #changing it to numeric
  EDGAR_d03 <- df_MAM[,11]
  EDGAR_d03 <- dplyr::pull(df_MAM,11)
  ...
  obs <- dplyr::pull(df_MAM, 2) #changing it to numeric
  ODIAC_d03 <- df_MAM[,13]
  ODIAC_d03 <- dplyr::pull(df_MAM,13)
...
write.xlsx2(finalDf, 
            file = "E:/Eeshan/evaluation/results/results_without_background_MAM_d01_d02_d03.xlsx", 
            sheetName="Average_6", 
            append=TRUE)
