...
for(j in 1:length(sheets)){
  ...
  df_MAM <- df %>% 
    filter(df$Time > as.POSIXct("2016-02-28 23:00:00", tz="UTC") & df$Time < as.POSIXct("2016-06-01 00:00:00", tz="UTC"))
  
  # reading in background concentration data for specific months
  df_MAM_ts <- df_ts %>% 
    filter(df_ts$Time > as.POSIXct("2016-02-28 23:00:00", tz="UTC") & df_ts$Time < as.POSIXct("2016-06-01 00:00:00", tz="UTC"))  
  ...
  # just for 2016 and 2016
  # omitting results from FFDAS and VULCAN
  df_MAM <- select(df_MAM, c(-FFDAS,-VULCAN,-FFDAS_d02,-VULCAN_d02))
...
write.xlsx2(finalDf, 
            file = "E:/Eeshan/evaluation/results/results_without_background_ts_MAM_d01_d02.xlsx", 
            sheetName= "2016", 
            append = TRUE)
