...
for(j in 1:length(sheets)){
  ...
  df_JJA_ts <- df_ts %>% 
    filter(df_ts$Time > as.POSIXct("2017-05-31 23:00:00", tz="UTC") & df_ts$Time < as.POSIXct("2017-09-01 00:00:00", tz="UTC"))
  
  time <- df_JJA[, 1]
  df_without_time <- df_JJA[, -1]
  
  df_ts_without_time <- df_JJA_ts[, -1]
  ...
  df_JJA <- data.frame(time, df_final)
  
  # omitting results from FFDAS and VULCAN
  df_JJA <- select(df_JJA, c(-FFDAS,-VULCAN,-FFDAS_d02,-VULCAN_d02,-FFDAS_d03,-VULCAN_d03))
  ...
  statDf[i,1] <- paste(sheets[j],'_JJA_d03')
...
write.xlsx2(finalDf, 
            file = "E:/Eeshan/evaluation/results/results_without_background_ts_JJA_d01_d02_d03.xlsx", 
            sheetName="2017", 
            append=TRUE)
