...
for(j in 1:length(sheets)){
  ...
  df_yearly_ts <- df_ts %>% 
    filter(df_ts$Time > as.POSIXct("2011-12-31 23:00:00", tz="UTC") & df_ts$Time < as.POSIXct("2018-01-01 00:00:00", tz="UTC"))  
  
  time <- df_yearly_initial[, 1]
  df_without_time <- df_yearly_initial[, -1]
  
  df_ts_without_time <- df_yearly_ts[, -1]
  ...
  df_yearly <- data.frame(time, df_final)
  
  # removing the na values
  df_yearly <- na.omit(df_yearly)
...
# d01  ##### 
  ...
  statDf[i,1] <- paste(sheets[j],'_yearly_d01')
...
# d02  ##### 
  obs <- df_yearly[,2]
  obs <- dplyr::pull(df_yearly, 2) #changing it to numeric
  EDGAR_d02 <- df_yearly[,7]
  EDGAR_d02 <- dplyr::pull(df_yearly,7)
  ...
  statDf[i,1] <- paste(sheets[j],'_yearly_d02')
  ...
  obs <- df_yearly[,2]
  obs <- dplyr::pull(df_yearly, 2) #changing it to numeric
  ODIAC_d02 <- df_yearly[,9]
  ODIAC_d02 <- dplyr::pull(df_yearly,9)
...
# d03  ##### 
  obs <- df_yearly[,2]
  obs <- dplyr::pull(df_yearly, 2) #changing it to numeric
  EDGAR_d03 <- df_yearly[,11]
  EDGAR_d03 <- dplyr::pull(df_yearly,11)
  ...
  statDf[i,1] <- paste(sheets[j],'_yearly_d03')
  ...
  obs <- df_yearly[,2]
  obs <- dplyr::pull(df_yearly, 2) #changing it to numeric
  ODIAC_d03 <- df_yearly[,13]
  ODIAC_d03 <- dplyr::pull(df_yearly,13)
...
write.xlsx2(finalDf, 
           file = "E:/Eeshan/evaluation/results/results_without_background_ts_yearly_d01_d02_d03.xlsx", 
           sheetName="Average_6", 
           append=TRUE)
