...
for(j in 1:length(sheets)){
  ...
  df_without_time <- df_Jan_initial[, -1]
  test<- df_without_time[] - 375 #substracting pre-industrial values 
  options(digits = 5) #changing the decimal places 
  df_Jan <- data.frame(time, test)
  df_Jan <- df_Jan[df_Jan$EDGAR >= 0, ] # drop negative values
  df_Jan <- df_Jan[df_Jan$ODIAC >= 0, ] # drop negative values
  df_Jan <- df_Jan[df_Jan$EDGAR_d02 >= 0, ] # drop negative values
  df_Jan <- df_Jan[df_Jan$ODIAC_d02 >= 0, ] # drop negative values  
  
  # just for 2016 and 2017
  # omitting results from FFDAS and VULCAN
  df_Jan <- select(df_Jan, c(-FFDAS,-VULCAN,-FFDAS_d02,-VULCAN_d02))
  ...
  columns <- colnames(df_Jan)
  obs <- df_Jan[,2]
  obs <- dplyr::pull(df_Jan, 2) #changing it to numeric
  
  EDGAR <- df_Jan[,3]
  EDGAR <- dplyr::pull(df_Jan,3)
  ...
  statDf[i,1] <- paste(sheets[j],'_Jan_d01')
  ...
  statDf[i,1] <- paste(sheets[j],'_Jan_d02')
  ...
  # VULCAN_d02 <- df_Jan[,10]
  # VULCAN_d02 <- dplyr::pull(df_Jan,10)
  # 
  # statsReady <- modeval(measured = obs,
...
write.xlsx2(finalDf, 
           file = "E:/Eeshan/evaluation/results/results_without_background_DJF_d01_d02.xlsx", 
           sheetName="2017", 
           append=TRUE)