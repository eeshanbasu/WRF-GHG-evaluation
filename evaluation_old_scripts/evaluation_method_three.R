...
for(j in 1:length(sheets)){
  ...
  statDf[i,1] <- paste(sheets[j],'_July_d01')
  ...
  # d02  ##### 
  EDGAR_d02 <- df_July[,7]
  EDGAR_d02 <- dplyr::pull(df_July,7)
  ...
  statDf[i,1] <- paste(sheets[j],'_July_d02')
  ...
  FFDAS_d02 <- df_July[,8]
  FFDAS_d02 <- dplyr::pull(df_July,8)
  ...
  ODIAC_d02 <- df_July[,9]
  ODIAC_d02 <- dplyr::pull(df_July,9)
  ...
  VULCAN_d02 <- df_July[,10]
  VULCAN_d02 <- dplyr::pull(df_July,10)
...
write.xlsx(finalDf, 
           file = "E:/Eeshan/evaluation/results/results_method_three_July.xlsx", 
           sheetName="July", 
           append=TRUE)
